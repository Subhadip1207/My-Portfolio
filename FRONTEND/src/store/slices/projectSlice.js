...
export const addNewProject = (formData) => async(dispatch) => {
    ...
    try {
        const {data} = await axios.post(`http://localhost:4000/api/v1/project/add`, formData, {withCredentials:true, headers:{'Content-Type':'multipart/form-data'}});
...
export const updateProject = (id,newData) => async(dispatch) => {
    ...
    try {
        const {data} = await axios.put(`http://localhost:4000/api/v1/project/update/${id}`, newData, {withCredentials:true , headers:{"Content-Type":"multipart/form-data"}});
